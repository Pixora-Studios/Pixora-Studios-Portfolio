        ...
        <div className="max-w-4xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-0 relative">
             ...
             <div className="space-y-3 md:pr-10">
                <div className="flex items-center space-x-2 text-red-500 font-display font-bold text-xl mb-6">
                   <X className="w-6 h-6" />
                ...
                {comparison.map((item, index) => (
                   <motion.div
                      ...
                      className="p-3.5 rounded-xl bg-red-500/5 border border-red-500/10 text-sm text-text-muted-light dark:text-text-muted-dark"
                   >
                      {item.without}
                   </motion.div>
                ))}
             ...
             <div className="space-y-3 md:pl-10 mt-6 md:mt-0">
                <div className="flex items-center space-x-2 text-emerald-500 font-display font-bold text-xl mb-6">
                ...
                {comparison.map((item, index) => (
                   <motion.div
                      ...
                      viewport={{ once: true }}
                      transition={{ delay: index * 0.05 }}
                      className="p-3.5 rounded-xl bg-emerald-500/5 border border-emerald-500/10 text-sm text-text-primary-light dark:text-text-primary-dark font-medium"
                   >
                      {item.with}
                   </motion.div>
                ))}
